    ...
    <div className="admin-form-group" style={{ marginBottom: '1.5rem' }}>
      ...
      {error && (
        <div style={{
          ...
          <span>{error}</span>
          <button type="button" onClick={() => setError('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', padding: 0 }}>
      ...
      {showUrlInput ? (
          ...
          {value && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <img
                src={value}
                alt="Preview"
                style={{ width: '48px', height: '48px', objectFit: 'cover', border: '1px solid var(--border-color)', borderRadius: '0' }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div
            ...
            style={{
              border: isDragging ? '1px dashed var(--accent)' : '1px dashed var(--border-color)',
            ...
            {isUploading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }} onClick={(e) => e.stopPropagation()}>
                <img
                  ...
                  style={{ width: '80px', height: '80px', objectFit: 'cover', border: '1px solid var(--border-color)', borderRadius: '0' }}
                />
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button type="button" className="admin-btn admin-btn-sm" onClick={triggerFileSelect} style={{ borderRadius: '0' }}>
                    Change
                  </button>
                  <button type="button" className="admin-btn admin-btn-sm admin-btn-danger" onClick={() => onChange('')} style={{ borderRadius: '0' }}>
                    Remove
                  </button>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', color: 'var(--text-muted)' }}>
                ...
                <span style={{ fontSize: '0.85rem' }}>Drag & drop image here, or <strong style={{ color: 'var(--accent)' }}>click to browse</strong></span>
                <span style={{ fontSize: '0.7rem', color: 'var(--text-3)' }}>Click this zone and press Ctrl+V to paste from clipboard</span>
              </div>
            )}
          </div>
        </div>
