...
export default function CrudSection({ sectionName, label, schema, token, apiUrl, authFetch }) {
  ...
  const handleBatchToggleActive = async (isActive) => {
    ...
    try {
      const res = await authFetch('/api/admin/batch-toggle-active', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: sectionName, ids: selectedIds, is_active: isActive })
      });
      const data = await res.json();
  ...
  const handleBatchDelete = async () => {
    setBatchLoading(true);
    try {
      const res = await authFetch('/api/admin/batch-delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: sectionName, ids: selectedIds })
      });
      const data = await res.json();
  ...
  const handleExportCsv = () => {
    const exportData = items.filter(item => selectedIds.includes(item.id));
    const columns = [
      { header: 'ID', key: 'id' },
      ...schema.map(field => ({ header: field.label, key: field.key })),
      { header: 'Status', key: (item) => item.is_active ? 'Active' : 'Inactive' }
    ];
    exportToCsv(exportData, columns, `${sectionName}_export`);
  };
  ...
  return (
    <div className="crud-section">
      ...
      {items.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', background: 'var(--bg-secondary)', padding: '12px', border: '1px solid var(--border)', borderRadius: '4px' }}>
          <BatchCheckbox
            checked={isSelectedAll(items)}
            indeterminate={isIndeterminate(items)}
            onChange={() => toggleSelectAll(items)}
          />
          <span style={{ fontSize: '13px', color: 'var(--text-2)' }}>Select All {items.length} {label}s</span>
        </div>
      )}
        ...
        {items.length === 0 ? (
          ...
          items.map((item, i) => {
            ...
            return (
              <div
                key={item.id}
                className={`crud-list-item ${isDragging ? 'dragging' : ''} ${isDragOver ? 'drag-over' : ''} ${selected ? 'row-selected' : ''}`}
                draggable
                ...
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  background: selected ? 'var(--bg-selected)' : 'var(--bg-secondary)',
                  padding: 'var(--space-3)',
      ...
      <BatchActionBar
        ...
        actions={[
          ...
          },
          {
            ...
            icon: 'trash',
            variant: 'danger',
            confirm: `Are you sure you want to delete ${selectionCount} items permanently?`,
